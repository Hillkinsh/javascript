...
Promise._immediateFn =
  ...
  // console.log(setImmediateFunc, "::Promise._immediateFn")
...
Promise.prototype.then = function (onFulfilled, onRejected) {
  ...
  */
  ...
  console.log('then....',prom) // Promise { _state: 0, _handled: false, _value: undefined, _deferreds: [] }，新期约
...
function finale(self) {
  ...
  if (self._state === 2 && self._deferreds.length === 0) {
       ...
       * 参数fn：要执行的警告方法
       * _immediateFn 是一个异步方法。在这里出现，没加catch会抛出错误问题。
       */
      Promise._immediateFn(function () {
        // console.log('_immediateFn cb..', self)
          /* 如果未被处理过，则给出警告 */
          if (!self._handled) {
              /**
                  ...
                  then.call(
                      ...
                      function (e) {
                          args[i] = {
                              status: 'rejected',
                              reason: e
                          } // 将该项重写为{status:'rejected',reason:错误原因}
